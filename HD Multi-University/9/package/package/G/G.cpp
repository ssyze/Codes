...
template <class T> struct Treap {
    ...
    void split(int u, const int left_size, int& x, int& y)
    {
        ...
        if (sz[lc[u]] + 1 <= left_size)
            x = u, split(rc[u], left_size - sz[lc[u]] - 1, rc[u], y);
        else
            y = u, split(lc[u], left_size, x, lc[u]);
        pushup(u);
    }
    void split_by_min(int u, const int right_mn, int& x, int& y)
    {
        if (!u) return x = y = 0, void();
        if (mn[rc[u]] < right_mn || val[u] < right_mn)
            x = u, split_by_min(rc[u], right_mn, rc[u], y);
        else
            y = u, split_by_min(lc[u], right_mn, x, lc[u]);
        pushup(u);
    }
    ...
    void build_from(int* a, int l, int r) { root = dfs(a, l, r); }
