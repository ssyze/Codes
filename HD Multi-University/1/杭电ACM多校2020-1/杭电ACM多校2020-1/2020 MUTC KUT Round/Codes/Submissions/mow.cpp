...
struct Point {
    ...
    T distance(const Point &rhs) const { return (rhs - *this).length(); }
    Vector operator*(const T &rhs) const { return Vector(x * rhs, y * rhs); }
    Vector operator/(const T &rhs) const { return Vector(x / rhs, y / rhs); }
    T operator*(const Vector &rhs) const { return x * rhs.y - y * rhs.x; }
    T operator&(const Vector &rhs) const { return x * rhs.x + y * rhs.y; }
    T cross(const Point &lhs, const Point &rhs) const { return (lhs - *this) * (rhs - *this); }
    int quadrant() const
    {
    ...
    Vector normal(const bool &rht = false) const { return rht ? Vector(y, -x) : Vector(-y, x); }
};
...
template <typename T>
Point<T> line_line_intersection(const Point<T> &p1, const Point<T> &p2, const Point<T> &q1,
                                const Point<T> &q2)
{
...
struct Line {
    ...
    T operator*(const Line &rhs) const { return (v - u) * (rhs.v - rhs.u); }
    T operator&(const Line &rhs) const { return (v - u) & (rhs.v - rhs.u); }
...
int main()
{
    ...
    for (cin >> ncase; ncase--;) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // cout << nml.x << ' ' << nml.y << endl;
            // cout << (p[i] + nml).x << ' ' << (p[i] + nml).y << endl;
            // cout << (p[(i + 1) % n] + nml).x << ' ' << (p[(i + 1) % n] + nml).y << endl;
