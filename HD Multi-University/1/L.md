2020Multi-University Hdu1 L.Mow

<!--index-menu-->

# 题意：

给定一个多边形区域要除草，手除单位面积价格为$a$，机器除单位面积价格为$b$，但机器除草范围为圆形且不能到多边形区域外，求除完草需要多少钱

其中机器不能到区域外为严格不能，即任何部分不能出区域
...
机器除草面积可以这样考虑，对多边形取向内收缩 $r$ 距离的多边形，求该多边形的面积，加上这个多边形周长$\times \ r$，加上 $\pi \cdot r^2$

一一解释以上操作

①取向内收缩距离为 $r$ 的多边形，这个操作可以由原多边形的每条线向中心平移 $r$ 个单位后求半平面交得到，这部分面积是显然机器可以除掉且不会出范围的最大的面积

②和③这部分面积是圆心在内多边形周长上移动时多边形外部的面积

②是取每条边上移动的四边形的面积

③是任意两条边上四边形会存在一个夹角，这些夹角加起来正好$360^\circ$（多边形外角和360

然后就做完了

（这道题最大的意义是让我终于把计算几何板子搞了搞，虽然还没搞彻底
